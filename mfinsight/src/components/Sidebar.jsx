  ...
  return (
    <aside style={{
      ...
      <div style={{
        ...
        {!collapsed && (
          <button onClick={() => setCollapsed(true)} style={{ background: "none", border: "none", color: "#64748b", cursor: "pointer", fontSize: 16, padding: 4 }}>
            ◀
          </button>
        )}
      </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "12px 0" }}>
        <div style={{ padding: collapsed ? "0 8px" : "0 12px" }}>
          {navItems.map(item => (
            <NavItem key={item.id} item={item} activePage={activePage} setActivePage={setActivePage} collapsed={collapsed} />
          ))}
        </div>
        <div style={{ marginTop: 16 }}>
          {!collapsed && (
            <div style={{ padding: "6px 20px", color: "#475569", fontSize: 11, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase" }}>
              DASHBOARDS
            </div>
          )}
          <div style={{ padding: collapsed ? "0 8px" : "0 12px" }}>
